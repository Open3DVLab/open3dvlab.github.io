...
function objectSceneEventDTU(idx) {
    ...
    for (let i = 0; i < imagesLength; i++) {
        ...
        if (imageContainer) {
            let image = imageContainer.getElementsByClassName('b-dics__image')[0];
            if (image) {
                let folders = ['scene01', 'scene02', 'scene03', 'scene04'];
                let imageFolder   = folders[idx];
                let filenames = ['pixel.png', 'mvs.png', 'ours.png', 'gt.png'];
                let imageFileName = filenames[i];
                image.src = `resources/compare_images/dtu/${imageFolder}/${imageFileName}`;
            }
        }
    }
    updateTabStates('compare_dtu', idx);
}
    ...
    for (let i = 0; i < imagesLength; i++) {
        ...
        if (imageContainer) {
            ...
            if (image) {
                ...
                let filenames = ['pixel.png', 'mvs.png', 'ours.png', 'gt.png'];
                let imageFileName = filenames[i];
                image.src = `resources/compare_images/re10k/${imageFolder}/${imageFileName}`;
            }
        }
    }
    updateTabStates('compare_re10k', idx);
}
